...
(() => {
  ...
  const FULL_NAMES={MLB:{ARI:'Arizona Diamondbacks',ATH:'Athletics',ATL:'Atlanta Braves',BAL:'Baltimore Orioles',BOS:'Boston Red Sox',CHC:'Chicago Cubs',CWS:'Chicago White Sox',CIN:'Cincinnati Reds',CLE:'Cleveland Guardians',COL:'Colorado Rockies',DET:'Detroit Tigers',HOU:'Houston Astros',KC:'Kansas City Royals',LAA:'Los Angeles Angels',LAD:'Los Angeles Dodgers',MIA:'Miami Marlins',MIL:'Milwaukee Brewers',MIN:'Minnesota Twins',NYM:'New York Mets',NYY:'New York Yankees',PHI:'Philadelphia Phillies',PIT:'Pittsburgh Pirates',SD:'San Diego Padres',SEA:'Seattle Mariners',SF:'San Francisco Giants',STL:'St. Louis Cardinals',TB:'Tampa Bay Rays',TEX:'Texas Rangers',TOR:'Toronto Blue Jays',WSH:'Washington Nationals'}};
  ...
  const fullName=(code,l)=>FULL_NAMES[l]?.[canonCode(code,l)]||window.TEAM_FULL_NAMES?.[l]?.[canonCode(code,l)]||window.TEAM_NAMES?.[l]?.[canonCode(code,l)]||shortName(code,l);
  ...
  window.gameDropdownName=function(value,l){
    const raw=clean(value),parts=raw.split('@');
    if(parts.length!==2)return raw;
    return `${fullName(parts[0],l)} @ ${fullName(parts[1],l)}`;
  };
  ...
  window.prepareGameOption=function(option,l){
    ...
    option.dataset.compactLabel=window.gameDisplayName(option.value,l)+original.suffix;
    option.dataset.fullLabel=window.gameDropdownName(option.value,l)+original.suffix;
  };
