...
(() => {
  ...
  function effectiveLeague(ticket,leg){return cleanText(leg?.league||ticket?.league).toUpperCase()}
  function ticketHasMlbOrLegacyLeg(ticket){return (ticket.legs||[]).some(leg=>{const league=effectiveLeague(ticket,leg);return !league||league==='MLB'})}
  async function evaluateRecord(record){const ticket=record.ticket||{},evaluated=[];for(const leg of ticket.legs||[]){const league=effectiveLeague(ticket,leg);if(league&&league!=='MLB'){evaluated.push({...leg,__live:result('UNAVAILABLE',null,leg.target,'—','Live support not added yet')});continue}const game=ticketGameForLeg(ticket,leg),date=ticketDateForLeg(ticket,leg);if(!game||!date){evaluated.push({...leg,__live:result('UNAVAILABLE',null,leg.target,'—','Missing game or date')});continue}try{const feed=await loadFeed(date,game);evaluated.push({...leg,__feed:feed,__live:evaluateMlbLeg(leg,feed)})}catch(e){evaluated.push({...leg,__live:result('UNAVAILABLE',null,leg.target,'—',e.message||'Live data failed')})}}return{...record,__evaluated:evaluated}}
